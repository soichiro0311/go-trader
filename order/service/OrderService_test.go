...
func TestRegisterOrder(t *testing.T) {
	...
	repMock := repository.NewBoardInfoRepositoryMock()
	cur1 := domain.NewCurrency("BTC")
	cur2 := domain.NewCurrency("USD")
	_, price := domain.NewPrice(cur1, cur2, 2900.555)
	repMock.Save(domain.BoardInfo{cur1, cur2, 100, price, "20190412123045"})
	var boardRep repository.BoardInfoRepository = repMock
	service := NewOrderService(orderRep, boardRep)
...
func TestOrders(t *testing.T) {
	...
	repMock.Save(domain.BoardInfo{cur1, cur2, 100, price1, "20190412123045"})
	_, price2 := domain.NewPrice(cur1, cur2, 1200.095)
	var boardRep repository.BoardInfoRepository = repMock
	service := NewOrderService(orderRep, boardRep)
	req1 := model.NewRegisterOrderRequest("BTC", "USD")
	service.RegisterOrder(*req1)
	repMock.Save(domain.BoardInfo{cur1, cur2, 100, price2, "20190412123050"})
